...
def menu():
    while 1:
        ...
        match ans:
            ...
            case 5:
                i = 1
                while i: # проверка, ввел ли пользователь то что нужно
                    firstPar = input("Введите первый параметр( простое число/ желательно большое для лучшей защиты/ для выбора можете использовать функцию (1) - Решето Эратосфена):\n")
                    secondPar = input("Введите второй параметр( простое число/ желательно большое для лучшей защиты/ для выбра можете использовать функцию (1) - Решето Эратосфена):\n")
                    if firstPar.isdigit() and secondPar.isdigit():
                        firstPar = int(firstPar)
                        secondPar = int(secondPar)
                        i = 0
                    else: 
            ...
            case 6:
               ...
               # cipher = input("Введите зашифрованное сообщение")
                plaintext = decryptRsa(keys[1], cipher)
